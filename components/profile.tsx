...
const verifier = process.env.NEXT_PUBLIC_WEB_3_AUTH_VERIFIER ?? "";
...
export default function UserInfo({ session }: UserInfoProps) {
  ...
  const isWeb3AuthConnected = useRef(false)
  const prevSessionIdToken = useRef<string | null>(null)  
  ...
  useEffect(() => {
    const initWeb3Auth = async () => {
      ...
      try {
        ...
        const isSessionIdTokenChanged = session.idToken !== prevSessionIdToken.current

        console.log({curr: session.idToken, prev: prevSessionIdToken.current})

        if(isSessionIdTokenChanged){ 
          isWeb3AuthConnected.current = false
        }

        if (web3auth.status === "connected" && !isSessionIdTokenChanged) {
          setProvider(web3auth.provider);
        } else if (session?.idToken && !isWeb3AuthConnected.current) {
          
          const { payload } = decodeToken(session.idToken);
          
          isWeb3AuthConnected.current = true
          prevSessionIdToken.current = session.idToken

          console.log("in login part")
          if (web3auth.connected) {
            console.log("need to logout first")
